...
import { 
  ...
  Loader2, Phone, Edit, X, Calendar, Search, Filter 
} from 'lucide-react'; 
...
const Dashboard = () => {
  ...
  const [filterDay, setFilterDay]   = useState(''); // Grouping by Day (1-31)
  ...
  const downloadBill = async (student) => {
    try {
      ...
      doc.text(`Joining Date: ${new Date(student.joiningDate).toLocaleDateString()}`, 15, 35);
      doc.text(`Total Due: RS ${student.totalDue}`, 15, 45);
  ...
  // ── STYLES ──
  const inp = { width: '100%', padding: '12px 14px', borderRadius: 10, border: `1.5px solid ${S.border}`, fontSize: 14, background: '#F8FAFF', outline: 'none', marginBottom: 10, boxSizing: 'border-box' };
  const ibtn = (variant) => {
    const map = { 
        ...
        edit: { background: '#f0f0f0', color: '#555' }
    };
  ...
  return (
    <div style={{ background: S.pageBg, minHeight: '100vh', fontFamily: "'Segoe UI', sans-serif", paddingBottom: 100 }}>
      ...
      <div style={{ background: S.white, padding: '18px 16px', borderBottom: `1px solid ${S.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 10 }}>
        <div style={{ fontSize: 17, fontWeight: 700, color: S.navy }}>Dashboard</div>
      ...
      <div style={{ display: 'flex', gap: '8px', padding: '12px', margin: '10px 12px', background: S.white, borderRadius: '12px', border: `1px solid ${S.border}` }}>
        <select value={viewMonth} onChange={(e) => setViewMonth(parseInt(e.target.value))} style={{ padding: 10, borderRadius: 8, border: '1px solid #ddd', flex: 1 }}>
          {months.map((m, i) => <option key={i} value={i}>{m}</option>)}
        </select>
        <select value={viewYear} onChange={(e) => setViewYear(parseInt(e.target.value))} style={{ padding: 10, borderRadius: 8, border: '1px solid #ddd', flex: 1 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 6, padding: '0 12px 14px' }}>
        <div style={{ background: S.white, borderRadius: 12, padding: '12px 5px', textAlign: 'center', borderBottom: `3px solid ${S.green}` }}>
          <div style={{ fontSize: 8, color: S.muted }}>UDHARI</div>
          <div style={{ fontSize: 11, fontWeight: 700, color: S.green }}>₹{totalRevenue}</div>
        </div>
        <div style={{ background: S.white, borderRadius: 12, padding: '12px 5px', textAlign: 'center', borderBottom: `3px solid ${S.red}` }}>
          <div style={{ fontSize: 8, color: S.muted }}>EXPENSE</div>
          <div style={{ fontSize: 11, fontWeight: 700, color: S.red }}>₹{totalExp}</div>
        </div>
        <div style={{ background: S.white, borderRadius: 12, padding: '12px 5px', textAlign: 'center', borderBottom: `3px solid ${S.navy}` }}>
        ...
        <div style={{ background: S.white, borderRadius: 12, padding: '12px 5px', textAlign: 'center', borderBottom: `3px solid ${S.blue}` }}>
        ...
        <div style={{ margin: '0 12px 14px', background: S.amberBg, borderRadius: 14, padding: '12px', border: `1px solid ${S.amberBorder}` }}>
          ...
          {alerts.map(s => (
            <div key={s._id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: S.white, padding: '10px', borderRadius: 8, marginBottom: 5 }}>
              ...
              <button onClick={() => handleEmailReminder(s)} style={{ fontSize: 11, background: S.red, border: 'none', color: S.white, fontWeight: 700, padding: '5px 10px', borderRadius: 5 }}>Email Alert</button>
            </div>
          ))}
        </div>
      )}
      ...
      <div style={{ margin: '0 12px 14px', background: S.white, borderRadius: 16, padding: 16, border: `1px solid ${S.border}` }}>
        ...
        <form onSubmit={handleAdd}>
          ...
          <input value={phone} onChange={e => setPhone(e.target.value)} placeholder="Phone (Login ID)" required style={inp} />
          ...
          <div key={s._id} style={{ background: S.white, borderRadius: 16, padding: 14, marginBottom: 10, border: `1px solid ${S.border}`, borderLeft: `5px solid ${s.totalDue > 1500 ? S.red : S.navy}` }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                ...
                <div style={{ display: 'flex', gap: 5 }}>
                    ...
                    <button onClick={() => handleEditClick(s)} style={ibtn('edit')}><Edit size={14} /></button>
                </div>
            </div>
            <div style={{ display: 'flex', gap: 4, justifyContent: 'space-between' }}>
              <button onClick={() => downloadBill(s)} style={ibtn('bill')}><Download size={14} /> Bill</button>
              <button onClick={() => sendWelcomeMessage(s)} style={ibtn('link')}><MessageCircle size={14} /> Link</button>
      ...
      {editingStudent && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: S.white, width: '100%', maxWidth: 400, borderRadius: 20, padding: 20 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
              <b style={{ fontSize: 18 }}>Edit Student Details</b>
            ...
            <label style={{ fontSize: 11, color: S.muted }}>Password/PIN:</label>
            <input value={editingStudent.password} style={inp} onChange={e => setEditingStudent({...editingStudent, password: e.target.value})} />
            <label style={{ fontSize: 11, color: S.muted }}>Email Address:</label>
            ...
            <button onClick={handleEditSave} style={{ width: '100%', padding: 14, background: S.green, color: S.white, border: 'none', borderRadius: 12, fontWeight: 700, marginTop: 10 }}>Save Changes</button>
          </div>
        </div>
      )}
