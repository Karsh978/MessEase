    ...
    <div style={{ background: S.pageBg, minHeight: '100vh', fontFamily: "'Segoe UI', sans-serif", paddingBottom: 100 }}>
      ...
      <div style={{ margin: '0 12px 14px', background: S.white, borderRadius: 16, padding: 16, border: `1px solid ${S.border}` }}>
        ...
        <form onSubmit={handleAdd}>
          ...
          <input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Password (e.g. 1234)" required style={inp} />
      ...
      {/* 👥 STUDENT LIST (With Profile Pic Feature) */}
          ...
          <div key={s._id} style={{ background: S.white, borderRadius: 16, padding: 14, marginBottom: 10, border: `1px solid ${S.border}`, borderLeft: `5px solid ${s.totalDue > 1500 ? S.red : S.navy}` }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
                ...
                <div style={{ 
                    ...
                    fontWeight: 700, fontSize: 15, overflow: 'hidden', border: `1px solid ${S.border}` 
                }}>
                    {s.profilePic ? (
      ...
      {editingStudent && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: S.white, width: '100%', maxWidth: 400, borderRadius: 20, padding: 22 }}>
            ...
            <input value={editingStudent.name} style={inp} onChange={e => setEditingStudent({...editingStudent, name: e.target.value})} placeholder="Name" />
            <input value={editingStudent.password} style={inp} onChange={e => setEditingStudent({...editingStudent, password: e.target.value})} placeholder="Password" />
            <input value={editingStudent.email} style={inp} onChange={e => setEditingStudent({...editingStudent, email: e.target.value})} placeholder="Email" />
            ...
            <button onClick={handleEditSave} style={{ width: '100%', padding: 14, background: S.navy, color: S.white, border: 'none', borderRadius: 12, fontWeight: 700 }}>Save Changes</button>
          </div>
        </div>
      )}
